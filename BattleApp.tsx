...
import { QuizBattleFSM, GameState, GameStateContext } from './utils/fsm';
...
export default function BattleApp() {
  const [gameState] = React.useState<GameState>('LOBBY');
  ...
  React.useEffect(() => {
    ...
    aiPlayers[0].rating = 1500;
    aiPlayers[1].rating = 1200;
    aiPlayers[2].rating = 1350;
    aiPlayers[3].rating = 1100;
  ...
  React.useEffect(() => {
    const unsubscribe = fsm.subscribe('LOBBY', (context: GameStateContext) => {
      console.log('State: LOBBY', context);
    });

    fsm.subscribe('MATCHMAKING', (context: GameStateContext) => {
    ...
    fsm.subscribe('ROUND', (context: GameStateContext) => {
    ...
    fsm.subscribe('GAME_OVER', (context: GameStateContext) => {
